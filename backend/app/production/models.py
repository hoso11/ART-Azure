...
# Valid stages for stock-based production batches (stored as plain strings on the
# row; kept as a constant rather than a DB enum so we never have to migrate
# Postgres enum types when the workflow changes).
VALID_BATCH_STAGES = {
