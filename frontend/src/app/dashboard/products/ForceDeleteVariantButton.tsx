...
const ERROR_MESSAGES: Record<string, string> = {
  insufficient_permissions:
    "Չունեք իրավասություն ուժով ջնջելու տարբերակը:",
  not_found: "Տարբերակը արդեն ջնջվել է:",
};

const FORCE_PHRASE = "FORCE DELETE";

export function ForceDeleteVariantButton({
  variantId,
  variantLabel,
  orderItemsCount,
  batchesCount,
  ...
  const handleForceDelete = async () => {
    ...
    try {
      const res = await clientFetch(
        `/products/variants/${variantId}/force`,
        { method: "DELETE" },
      );
      if (res.status === 204) {
        showToast("Տարբերակը ուժով ջնջվեց", "success");
        reset();
        router.refresh();
        return;
      }
      ...
      const msg =
        ...
        "Չհաջողվեց ուժով ջնջել տարբերակը";
      showToast(msg, "error");
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <>
      <button
        ...
        title="Ուժով ջնջել տարբերակը (առանց պաշարի վերականգնման)"
      >
        Ուժով ջնջել
      </button>
      {open && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-lg shadow-lg border-2 border-red-600 p-6 max-w-lg w-full">
            ...
            <ul className="text-sm text-gray-800 list-disc pl-5 mt-2 space-y-1">
              <li>
                Տարբերակը <strong>{variantLabel}</strong> ընդմիշտ կջնջվի:
              </li>
              {orderItemsCount > 0 && (
                <li>
                  <strong>{orderItemsCount}</strong> պատվերի տող կպահպանվի, բայց
                  տարբերակի անունը կնշվի որպես <em>ջնջված</em>:
                </li>
              )}
              {batchesCount > 0 && (
                <li>
                  <strong>{batchesCount}</strong> արտադրությունը կպահպանվի, բայց
                  տարբերակի անունը կնշվի որպես <em>ջնջված</em>:
                </li>
              )}
              <li>
                Պահեստի շարժերի պատմությունը <strong>կպահպանվի</strong>:
              </li>
              <li>
                Վաճառվող և խոտանի պաշարը <strong>ՉԻ</strong> վերականգնվի:
              </li>
              <li>
                Պատմական հաշվետվությունները կարող են ցուցադրել <em>(ջնջված)</em>{" "}
                հղումներ:
              </li>
              <li>
