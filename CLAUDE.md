...
- **Authorization** — five roles on `User.role` (since v35, migration `012_extend_user_roles`): `admin`, `director`, `production_manager`, `warehouse_manager`, `simple_user`. Enforce via FastAPI dependencies from `app.dependencies`: `get_current_user`, `require_authenticated`, `require_admin` (legacy admin-only routes), and `require_roles(*allowed)` (the canonical pattern for new code — composes with role group constants `ADMIN_ONLY`, `USER_MANAGEMENT`, `BUSINESS_MANAGER`, `INVENTORY_MANAGER`, `PRODUCTION_INVENTORY_READ`, `CUSTOMER_MANAGEMENT`, `REPORT_MANAGEMENT`, `PRODUCTION_REPORTS`, `ACTIVITY_VIEW`, `ORDERS_VIEW`, `PRODUCTS_VIEW`). A `simple_user` is linked 1:1 to a `Customer` via `User.customer_id`; any service that lists orders/data for a `simple_user` actor **must** filter by `current_user.customer_id`. The B2B split of `User` ↔ `Customer` is load-bearing — do not merge them. **Backend is authoritative**: every API endpoint enforces authorization; the frontend's `MODULE_ACCESS` table in `frontend/src/lib/permissions.ts` only hides UI elements (UX). Director can create/manage only `simple_user` accounts (`can_assign_role` / `can_manage_user` in `backend/app/users/service.py`); self-role-change and self-deactivation are blocked; last-active-admin demotion / deactivation returns 422 `last_admin_required`. See `handoff/CURRENT_STATE.md` "RBAC matrix — load-bearing" for the per-module access matrix and `handoff/SAFE_TASK_RULES.md` "RBAC — five-role authorization matrix" for the guard-implementation rules. The `userrole` Postgres ENUM is forward-only; rollback below v35 requires demoting any non-original-role rows first — see `handoff/ROLLBACK_NOTES.md` "RBAC ENUM extension is forward-only".
...
- **Auth gating is server-side.** `lib/auth.ts` exposes `getSession` (nullable), `requireAuth` (redirects to `/login`), `requireAdmin` (redirects non-admins to `/dashboard`), `requireRoles(...allowed)` (redirects when actor's role isn't in the allowed list — preferred for new pages), and `requireModule(module)` (redirects when `canAccessModule` returns false — driven by `MODULE_ACCESS` in `lib/permissions.ts`). The dashboard `layout.tsx` calls `requireAuth` so every nested route is guarded at render time. Per-page role gating goes inside the page via `requireModule(...)` / `requireRoles(...)` — backend RBAC is still authoritative; the helpers exist so non-permitted roles get redirected before they see a 403.
