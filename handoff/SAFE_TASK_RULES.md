...
## Production batch creation — single + bulk (v36)

17j. **Atomic, all-or-nothing for `POST /production/batches/bulk`.** Validation runs before any DB write; deductions and inserts only happen after every check has passed. Any later failure raises `ValidationException` and `get_db` rolls back the whole transaction, including freshly-inserted `StockMovement` rows. **Do not reorder** the service to deduct materials before validation completes, do not catch validation exceptions partway through to "rescue" earlier inserts, and do not switch to per-item commits — Option B (partial success) was explicitly rejected for this task.
17k. **Aggregated material deduction must produce ONE `StockMovement` per material**, not one per batch. The bulk service sums `quantity_per_item * quantity_to_produce` across all items per `material_id` and emits a single negative movement per material with reason `stock_based_production`. Per-batch movements would corrupt the inventory ledger and double-count under any future analytics that group by movement.
17l. **Reuse the existing `StockMovementReason.stock_based_production` enum value.** Do not introduce a new reason value (e.g. `stock_based_production_bulk`) — `StockMovementReason` is a Postgres ENUM and extending it is a forward-only migration just like `userrole` (see `KNOWN_RISKS.md` #14 and `ROLLBACK_NOTES.md` "RBAC ENUM extension is forward-only"). The bulk task explicitly avoided that path.
17m. **Pydantic enforces the bulk caps; the service rechecks defensively.** `ProductionBatchBulkCreate.items` has `min_length=1, max_length=50` and a `field_validator` that rejects duplicate `variant_id`. The service repeats these checks (raising typed `ValidationException`) so direct in-process callers (tests, scripts) can't bypass them. Do not weaken either layer.
17n. **Bulk endpoint reuses the `BUSINESS_MANAGER` gate** (admin / director / production_manager). Do not narrow it to admin-only without explicit approval — director already had bulk-create access at v35/v32 launch (canary `art@art.am`).
17o. **No server-side idempotency key today.** See `KNOWN_RISKS.md` #15. Don't loosen the frontend mitigations (submit disabled in-flight, modal closes on 2xx, 50-item cap) without first adding `Idempotency-Key` support.
