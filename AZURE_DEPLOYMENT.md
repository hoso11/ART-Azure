...
| `art-backend:v35` | Retired | RBAC — five-role authorization matrix. Extends `UserRole` enum with `director`, `production_manager`, `warehouse_manager`. Migration `012_extend_user_roles` adds the new ENUM values via `ALTER TYPE userrole ADD VALUE IF NOT EXISTS …` (non-destructive, forward-only). Centralized `require_roles(*allowed)` factory + role-group constants in `app.dependencies`. Per-route guards rewritten across all routers. Service helpers `can_assign_role` / `can_manage_user` / `is_last_active_admin`. New error codes: `role_assignment_denied` (403), `user_management_denied` (403), `self_role_change_denied` (403), `self_deactivation_denied` (403), `last_admin_required` (422). Tests: `backend/tests/test_authorization_matrix.py` (51 passed, 2 skipped). |
| `art-backend:v36` | **Live** | Bulk / series production batch creation. New endpoint `POST /api/v1/production/batches/bulk` (gate: `BUSINESS_MANAGER`) creates up to 50 `ProductionBatch` rows in one atomic transaction. Pydantic `ProductionBatchBulkCreate` enforces `1 <= len(items) <= 50`, no duplicate `variant_id`, `quantity_to_produce >= 1`. Service `create_production_batches_bulk` validates everything first, then aggregates material requirements across the whole request and deducts **once per material** (one `StockMovement` per material — not per batch — using the existing `stock_based_production` reason; **no migration**). Single-create endpoint untouched. Audit: one `production.batch_created` row per created batch. New error codes: `variant_not_found_or_wrong_product` (422), `duplicate_variant` (422), `empty_items` / `too_many_items` (422 — defensive). Tests: 16 new cases in `backend/tests/test_production_batches.py`. |
...
| `art-frontend:v32` | Retired | RBAC — sidebar / dashboard route / user-form gating per role. New `frontend/src/lib/permissions.ts` (single source of truth: `MODULE_ACCESS`, `canAccessModule`, `canAssignRole`, `allowedRolesFor`, `ROLE_LABELS`). `requireRoles(...)` and `requireModule(module)` helpers added to `frontend/src/lib/auth.ts`. Sidebar / TopHeader / MobileNav driven by `canAccessModule` and `ROLE_LABELS`. Dashboard route guards migrated from binary admin/user to per-module / per-role checks. User-create and user-edit forms restrict the role `<Select>` options via `allowedRolesFor(actor)`; the role select is disabled on self-edit (UI hint matching the backend self-role-change guard). New Armenian labels: `Տնօրեն`, `Արտադրության ղեկավար`, `Պահեստապետ`. |
| `art-frontend:v33` | **Live** | Bulk production modal. `frontend/src/app/dashboard/production/CreateBatchModal.tsx` adds a Single ↔ Շարք toggle. Single mode is unchanged. Շարք mode after a product is selected: per-variant table (checkbox / `Չափս` / `Գույն` / `Մնացորդ` / `Քանակ`) + common-quantity input + `Կիրառել ընտրվածներին` button + `Ընտրել ըստ գույնի` helper + footer counter. Submit disabled while no variants selected, any selected qty < 1, count > 50, or request in flight; modal closes on 2xx (prevents double-submit). New Armenian strings: `Մեկական`, `Շարք`, `Ընդհանուր քանակ`, `Կիրառել ընտրվածներին`, `Ընտրել ըստ գույնի`. Submits to `POST /production/batches/bulk`. |
...
Each feature below is part of the live deployment as of v33 (frontend) / v36 (backend):
...
| Bulk production batch creation | Browser: `/dashboard/production` → `Նոր արտադրություն` modal → toggle to **Շարք** → pick a product → check 2+ variants → set common quantity → `Կիրառել ընտրվածներին` → `Ստեղծել`. Toast `Ստեղծվեց N արտադրություն`; the new batches appear in the production list. Backend smoke (any cookie with `BUSINESS_MANAGER`): `curl -X POST $(terraform output -raw backend_url)/api/v1/production/batches/bulk -b cookies.txt -H "Content-Type: application/json" -H "Origin: $(terraform output -raw frontend_url)" -H "Referer: $(terraform output -raw frontend_url)/dashboard/production" -d '{"product_id":<id>,"items":[{"variant_id":<id>,"quantity_to_produce":1}]}'` returns 201 with `{"items": [...]}`. Validation probes: empty items → 422 `min_length`; duplicate `variant_id` → 422 `duplicate variant_id`; wrong product → 422 `variant_not_found_or_wrong_product`; insufficient aggregated material → 422 `insufficient_materials`. Inventory: `/inventory/materials/{id}/movements` shows **one** new movement per material per bulk request, not one per item. |
